...
// GenContextP2
func (s *sGenerator) genContextP2(ctx context.Context, key string, private_key2, public_key string) error {
...
func (s *sGenerator) calZKProofP2(ctx context.Context, key string, p1_hash_proof string) error {
...
func (s *sGenerator) calPublicKey2(ctx context.Context, key string, p1_zk_proof string) error {
	context_p2, err := s.FetchContextp2(ctx, key)
...
func (s *sGenerator) calRequest(ctx context.Context, key string, request string) (string, error) {
	context_p2, err := s.FetchContextp2(ctx, key)
	context_p2 = service.Sign().SignRecvRequestP2(context_p2, request)
	s.RecordContextp2(ctx, key, context_p2)

	s.RecordRequest(ctx, key, request)

	return context_p2, err
}

// 9.signature
func (s *sGenerator) CalSignTask(ctx context.Context, key string, msg string, request string) error {
	context_p2, err := s.FetchContextp2(ctx, key)
	if request != "" {
		context_p2, err = s.calRequest(ctx, key, request)
		if err != nil {
			return err
		}
	}
